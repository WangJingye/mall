...
class DataBase extends \ObjectAccess
{
    ...
    public static function getNewInstance($database = [])
    {
        ...
        $instance = new \PDO("mysql:host={$database['hostname']};dbname={$database['database']};port={$database['port']}", $database['username'], $database['password']);
        $instance->query("SET NAMES " . $database['charset']);
        return $instance;
    }
}

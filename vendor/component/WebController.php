...
class WebController extends \Controller
{
     ...
     * @return string
     */
    public function paginationJs($service)
    {
        $pageCount = 8;
        $leftCount = $rightCount = (int)(floor($pageCount / 2));
        //偶数时
        ...
        $params = \App::$request->params->toArray();
        $params['pageSize'] = $service->pageSize;
        $html = '<div class="pagination-list"><div class="page-container"><span style="padding-top: 0.3rem;">共' . $service->total . '条</span>';
        if ($endPage > 1) {
            $params['page'] = $service->page == 1 ? 1 : $service->page - 1;
            $html .=
                '<ul class="pagination">';
            if ($service->page > 1) {
                $html .= '<li class="page-item">' .
                    '<a class="page-link" data-key="' . $params['page'] . '" aria-label="Previous">' .
                    '<span aria-hidden="true">&laquo;</span>' .
                    '<span class="sr-only">Previous</span>' .
                    '</a></li>';
                $params['page'] = 1;
                $html .= '<li class="page-item ' . ($service->page == 1 ? 'disabled' : '') . '">' .
                    '<a class="page-link" data-key="' . $params['page'] . '">首页</a></li>';
            }
            for ($i = $startPage; $i <= $endPage; $i++) {
                $params['page'] = $i;
                $html .= '<li class="page-item ' . ($service->page == $i ? 'active' : '') . '"><a class="page-link" data-key="' . $params['page'] . '">' . $i . '</a></li>';
            }
            if ($service->page < $service->totalPage) {
                $params['page'] = $service->totalPage;
                $html .= '<li class="page-item ' . ($service->page == $service->totalPage ? 'disabled' : '') . '">' .
                    '<a class="page-link" data-key="' . $params['page'] . '">末页</a></li>';
                $params['page'] = $service->page == $endPage ? $endPage : $service->page + 1;
                $html .= '<li class="page-item ' . ($service->page == $endPage ? 'disabled' : '') . '">' .
                    '<a class="page-link" data-key="' . $params['page'] . '" aria-label="Next">' .
                    '<span aria-hidden="true">&raquo;</span>' .
                    '<span class="sr-only">Next</span>' .
                    '</a></li></ul></div>';
            }
     ...
     * @throws \Exception
     */
    public function renderPartial($uri)
    {
        $route = $uri != '' ? explode('/', trim($uri)) : [];
        if (count($route) != 3) {
            throw new \Exception('render route error');
        }
        ob_start();
        ob_implicit_flush(0);
        $view = APP_PATH . $route[0] . '/views/' . $route[1] . '/' . strtolower(trim(preg_replace('/([A-Z])/', '-$1', $route[2])) . '.php');
        include $view;
        return ob_get_clean();
    }
    ...
    {
        if (\App::$request->isAjax()) {
            return parent::success($message, $data);
        }
    }

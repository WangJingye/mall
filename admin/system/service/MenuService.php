...
class MenuService extends BaseService
{
    ...
    public function getAdminMenus()
    {
        ...
        if (\App::$user['identity'] == 0) {
            ...
            $selector2 = \Db::table('Menu')->field(['id', 'parent_id']);
            foreach (\App::$config->actionWhiteList as $route => $actions) {
            ...
            $lasts = $selector2->where(['status' => 1, 'depth' => 3])->findAll();
            $middles = [];
            $tops = [];
            if (count($lasts)) {
                ...
                $middles = \Db::table('Menu')->field(['id', 'parent_id'])->where(['id' => ['in', $lasts], 'depth' => 2])->findAll();
            }
            if (count($middles)) {
                $tops = \Db::table('Menu')->field(['id'])->where(['id' => ['in', array_column($middles, 'parent_id')], 'depth' => 1])->findAll();
                $roleMenus = array_merge($roleMenus, $lasts);
            }
            if (count($tops)) {
                $roleMenus = array_merge($roleMenus, array_column($tops, 'id'));
            }
            if (count($roleMenus)) {
    ...
    public function saveMenu($data)
    {
        ...
        if (!empty($data['parent_id'])) {
            $parent = \Db::table('Menu')->where(['id' => $data['parent_id']])->find();
        ...
        if (isset($data['id']) && $data['id']) {
            ...
            $data['id'] = \Db::table('Menu')->insert($data);
        }
